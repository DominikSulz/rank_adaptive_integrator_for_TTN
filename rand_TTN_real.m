function [Y,tau] = rand_TTN_real(n_l,r_l,r_tau)
...
tau1{1} = orth(rand(n_l(1),r_l(1)));
tau1{2} = orth(rand(n_l(3),r_l(3)));
tau1{3} = orth(rand(n_l(5),r_l(5)));
Y{1} = tau1;
% tree structure
tau{1} = cell(1,3);
...
tau2{1} = orth(rand(n_l(4),r_l(4)));
tau2{2} = orth(rand(n_l(2),r_l(2)));
Y{2} = tau2;
% tree structure
tau{2} = cell(1,2);

% tau3
Y{3} = orth(rand(n_l(6),r_l(6)));
